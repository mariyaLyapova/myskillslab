...
# MySkillsLab Docker Runner Script
# This script provides easy commands to manage the MySkillsLab Docker application
...
print_header() {
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE}  MySkillsLab Docker Manager${NC}"
    echo -e "${BLUE}================================${NC}"
}
...
build() {
    print_status "Building MySkillsLab Docker image..."
...
start() {
    print_status "Starting MySkillsLab application..."
    create_directories
    docker-compose up -d
...
stop() {
    print_status "Stopping MySkillsLab application..."
    docker-compose down
...
restart() {
    print_status "Restarting MySkillsLab application..."
...
status() {
    ...
    docker-compose exec myskillslab curl -f http://localhost:8081/actuator/health 2>/dev/null || print_warning "Health check failed"
}
...
clean() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        print_status "Cleaning up MySkillsLab Docker resources..."
...
shell() {
    print_status "Entering MySkillsLab container shell..."
    docker-compose exec myskillslab sh
}
...
backup() {
    ...
    docker-compose exec myskillslab cp /app/db/myskillslab.db /app/db/backup_${timestamp}.db
