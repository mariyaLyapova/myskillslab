...
./import-questions.sh -f input/myskillslab-questions-test.json
...
```properties
# Database connection
spring.datasource.url=jdbc:sqlite:db/myskillslab.db
...
- **Database Output**: `db/myskillslab.db` (auto-generated)
...
│   └── MySkillsLabApplication.java     # Main application
├── src/main/resources/
...
```bash
mvn clean package
java -jar target/myskillslab-app-0.0.1-SNAPSHOT.jar
```
...
```properties
logging.level.com.myskillslab=DEBUG
```
