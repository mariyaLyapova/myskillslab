...
This guide will help you deploy your MySkillsLab application to Google Cloud Run with BigQuery as the database.
...
```bash
# Create the dataset
bq mk --dataset --location=us-central1 YOUR_PROJECT_ID:myskillslab_db
...
gcloud builds submit --tag gcr.io/YOUR_PROJECT_ID/myskillslab-app:latest .
```

### Step 7: Deploy to Cloud Run

```bash
gcloud run deploy myskillslab-app \
  --image gcr.io/YOUR_PROJECT_ID/myskillslab-app:latest \
  --platform managed \
  --region us-central1 \
...
```bash
gcloud run services describe myskillslab-app \
  --platform managed \
  --region us-central1 \
...
bigquery.dataset=myskillslab_db
...
bq ls myskillslab_db

# Query questions
bq query --use_legacy_sql=false \
  'SELECT * FROM `YOUR_PROJECT_ID.myskillslab_db.questions` LIMIT 10'

# Count questions
bq query --use_legacy_sql=false \
  'SELECT COUNT(*) FROM `YOUR_PROJECT_ID.myskillslab_db.questions`'
```
...
```bash
# Real-time logs
gcloud run services logs tail myskillslab-app --region=us-central1

# Recent logs
gcloud run services logs read myskillslab-app --region=us-central1 --limit=50
```
...
```bash
gcloud run services update myskillslab-app \
...
gcloud run services delete myskillslab-app --region=us-central1

# Delete Docker images
gcloud container images delete gcr.io/YOUR_PROJECT_ID/myskillslab-app:latest

# Delete BigQuery dataset (WARNING: This deletes all data!)
bq rm -r -f -d myskillslab_db
```
...
Your MySkillsLab app is now running on Google Cloud Run with BigQuery!

**Your app URL**: Check the output of the deployment script or run:
```bash
gcloud run services describe myskillslab-app --region=us-central1 --format='value(status.url)'
```
